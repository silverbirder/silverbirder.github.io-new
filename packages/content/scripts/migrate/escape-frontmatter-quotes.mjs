...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/escape-frontmatter-quotes.mjs [--src <dir>] [--dest <dir>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const escapeSingleQuotesInSummary = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (firstQuote === -1 || lastQuote <= firstQuote) continue;

    const before = line.slice(0, firstQuote + 1);
    const value = line.slice(firstQuote + 1, lastQuote);
    const after = line.slice(lastQuote);
...
for (const fileName of markdownFiles) {
  ...
  const { content: escaped, changed } = escapeSingleQuotesInSummary(content);
  ...
  await writeFile(destPath, escaped, "utf8");
